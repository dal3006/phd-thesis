...
\item $q(\ell, t)$: An optional particle injection rate (or source term). If this term is omitted 
      ...
      $f(\ell_{max}, t)$ must be specified to solve \cref{eq:raddiffusion}. 
\end{enumerate}
%
...
\] 
is a normalisation term known as the \emph{model evidence}.
...
space-time domain. We seek a linear estimator for $f$ of the form 
$\hat{f}(x) = w^{T}\varphi(x) + b$, where $\varphi(.): \mathbb{R}^{2} \rightarrow \mathbb{R}^{d}$ 
is a $d$ dimensional feature map and $b$ is a scalar intercept.
...
sampled via adaptive Metropolis, the first $2000$ samples are kept aside as the \emph{burn in} 
