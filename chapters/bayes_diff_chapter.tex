...
\end{align*}
%
Given fixed values for $\gamma_{o}$, $\gamma_{c}$ and PDE parameters $\theta$, the equation above 
expresses the Lagrangian of the system in \cref{eq:surrogate}. The quantities 
$\alpha_{1}, \cdots, \alpha_{n_{o}}$ and $\beta_{1}, \cdots, \beta_{n_{c}}$ are the 
...
\end{equation}
%
The components of the symmetric block matrix system on the left hand side of \cref{eq:solution} are: 
\begin{enumerate}
...
\end{enumerate}
%
The surrogate model can now be used to estimate the phase space density at a point $x = (l,t)$.
%
...
%
For our problem we choose a basis which is a product of a Chebyshev basis of maximum degree $10$ in 
space and a Laguerre basis of maximum degree 4, in time.
%
...
for $\gamma_c$ works better in practice. For the experiment outlined in section \S~\ref{sec:exp}, 
...
\end{equation}
%
The surrogate model (\cref{eq:model}) gives the mean value for the phase space density, we use a 
hybrid RBF covariance function 
\[
  K(x_{i}, x_{j}) = \sigma^2 \exp(-\frac{1}{2} (\frac{|l_i - l_j|^2}{s} + \frac{|t_i - t_j|}{r}))  
\] 
to quantify the covariance of the phase space density $f$ over two points $x_i = (l_i, t_i)$ and 
$x_j = (l_j, t_j)$ in the domain. The values of $s$ and $r$, the length scales of the covariance 
...
For the purposes of the experiment, the parameters of $\kappa$ and $\lambda$ fixed while MCMC 
...
such that 100 points lie in the interior of the domain and $30$ points at the initial time step 
...
The posterior inferred for parameters of the particle injection rate $Q$ in 
