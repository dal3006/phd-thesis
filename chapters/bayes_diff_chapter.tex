...
electrons and ions in complex electromagnetic orbits  \cite{vanAllen}. Since their discovery,
the belts have been the subject of intensive research due to their complex behavior
and damaging effects on spacecraft \cite{GUBBY20021723, WellingSatellite, baker2002}.
...
radiation belt dynamics \cite{schulz2012particle}.
...
so called Roederer $L^{*}$ \cite{Roederer1970}. It is analogous to radial 
...
(\cite{Roederer1970}, \cite{JGR:JGR4463}).
...
differential equation (\ref{eq:raddiffusion}) known as \emph{radial
  diffusion} in the radiation belt community\cite{JGRA:JGRA9345}.
...
specified. It is a common practice (see \cite{GRL:GRL10762},
\cite{JGRA:JGRA15067}, \cite{JGRA:JGRA18021} and
\cite{GRL:GRL22815}) to parameterise the diffusion field
...
\cite{MEHRKANOON2015105}, \cite{MEHRKANOON20122502} as well as
parameter estimation of delay differential equations
\cite{MEHRKANOON2014830}, the approach taken in the aforementioned
research (\cite{MEHRKANOON2014830}) was expressing the parameter
...
(\cite{AMINATAEI20082887}, \cite{DUAN200866}, \cite{DUAN2006394} \& \cite{CNM:CNM419}),
as well as the Poisson control problem \cite{Pearson2013}. Further applications of RBFs include 
atmospheric flow \cite{Tillenius2015406}, convection-diffusion \cite{Safdari-Vaighani2015}, and 
Schrödinger's equation \cite{doi:10.1137/120893975}. See ref. \cite{fornberg2015} for a recent 
textbook with geoscience applications.

\paragraph{Gaussian Processes}

Gaussian Process (GP) models \cite{Rasmussen:2005:GPM:1162254} have a rich theory which has much overlapping 
with linear systems and deterministic and stochastic differential equations. 

\cite{Skilling1992} presented one of the earliest works which focused on calculating solutions of ordinary 
differential equations (ODE) systems with Gaussian Process methodology, \cite{Graepel} applied it for solving 
linear partial differential equations with Dirichlet and Von Neumann boundary conditions.

Interplay between linear operators and GP models applied to Bayesian filtering was investigated by \cite{Sarkka2011}. 
\cite{pmlr-v31-dondelinger13a} proposed an adaptive gradient matching technique to used Gaussian Process models for 
...
\cite{Lagaris}, \cite{Aarts2001} \&  \cite{TSOULOS20092385}. \cite{Baymani2011} used Feedforward networks for 
...
We employ the adaptive Metropolis algorithm as proposed by \cite{haario2001}, for sampling
...
The evolution of the Kp index \cite{BartelsKp} is assumed such that it mimics a geomagnetic storm event. 
