...
\begin{align}
 ...
 y_{m+\tau(x_m)} &= f(||x_m||) \label{eq:outputs}
\end{align}
%
...
\\\\
%
{\bf Problem I}: Constant time lag $\tau(x_m) = 5$, $y_{m+\tau(x_m)} = k ||x_m||^2 + c$\\
%
In all other problems, the time lag $\tau(x_m)$ depends on the velocity $v_m = k ||x_m||^2 + c$\\
%
{\bf Problem II}: Constant velocity $y_{m+\tau(x_m)} = v_m$, $\tau(x_m) = 100/v_m,\ k = 1,\ c = 10$\\
%
{\bf Problem III}: Constant acceleration 
$y_{m+\tau(x_m)} = \sqrt{v^2_{m} + 2ad}$, 
