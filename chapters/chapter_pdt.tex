...
As said, Eqs \ref{eq:pb1}-\ref{eq:pb2} define a regression problem that differs from standard regression 
...
to be continuous. % and \emph{bijective}. I remove this, which is not consistent with the model
...
\sum_{i \in T} \tau_i = 1.
\end{equation}
From constraint~(\ref{eq:cs1}), probability distribution $\hat p(x)$ thus is sought as a set of $\hat p_i(x)$ 
