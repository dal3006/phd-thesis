...
advantages of the probabilistic forecast provided by a GP. We use the so called 
\emph{long short-term memory network} (LSTM) \citep{hochreiter1997long} to provide a single point 
...
The remainder of this chapter is organised as follows: section \S~\ref{sec:datagpnn} presents the 
data used in this study, section \S~\ref{sec:methodgpnn} describes the computational method and 
how the LSTM and its combination with the GP are developed and optimised. Section 
\S~\ref{sec:resultsgpnn} presents the performance of the LSTM model when producing point 
predictions for the Dst one to six hours ahead and the evaluation of the probabilistic forecasts 
provided by the GPNN model. 
...
The \emph{long short-term memory} network (LSTM) belongs to the family of 
...
section \S~\ref{sec:datagpnn}. We also use the time history of Dst, from one to six hours back.
%
...
process component outlined in section \S~\ref{sec:gpcomponent}.
...
section \S~\ref{sec:osaGPmethod} for a detailed review of Gaussian process models and their 
...
In this chapter, we have presented a hybrid model for Dst forecasting, the GPNN, based on LSTM 
