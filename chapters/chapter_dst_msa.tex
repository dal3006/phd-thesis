...
Correspondingly the input gate (\cref{eq:inputgate}) decides what information is retained, depending 
...
time history of Dst, this emphasises the performance benefits of using exogenous data when predicting geomagnetic activity 
...
of $1$ (belonging to that storm class) if the probability predicted by the GPNN model is greater than the chosen threshold, 
and $0$ if otherwise. 
...
occurrence probability is $35\%$. For example, when there is $80\%$ predicted chance of a storm, 
...
and intense storms, but for six hour ahead prediction of superstorms, misjudgment is possible when the acceptance threshold increased. 
