...
Figure \ref{fig:lstmcell} represents a LSTM cell. The two elements fundamental to this cell are 
...
\begin{equation}\label{eq:cc}
 CC = Cov \left( Dst, \hat{D}st \right) / \sqrt[]{Var \left( Dst \right) Var \left( \hat{D}st \right) }
\end{equation}


We trained 6 LSTM models corresponding to forecasts from one to six hours ahead. This was implemented using the 
Lasagne library in Python (\url{http://lasagne.readthedocs.io/en/latest/index.html}). We thus obtained a 
vector of outputs \( NN \left( x \right)  \), for every input \( x \), which serves as the mean function 
for the Gaussian process component outlined below. 
...
\begin{equation}\label{eq:gpcov}
 k_{NN} \left( x, x' \right) = \frac{2}{ \pi } \text{sin}^{-1} \left( \frac{2x^{T}x'}{\sqrt{ \left( 1+2x^{T} x \right) }\sqrt{ \left( 1+2x'^{T}x' \right)}} \right)
\end{equation}
