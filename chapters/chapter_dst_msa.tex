...
ahead forecast, GPNN-GPS provides a CC of $0.873$ and a RMSE of $\SI{9.86}{\nano\tesla}$, while \citet{Lazzus} 
...
\begin{figure}
	...
	\caption{$\mathrm{Dst}$ predictions made by GPNN and GPNN-GPS for the 2003 Halloween storm.}
    \label{fig:gpnngpshalloween}
\end{figure}


\citet{wu1997geomagnetic} provided for a 6h ahead forecast a CC of $0.82$ and a RMSE of $\SI{20.8}{\nano\tesla}$, showing 
in comparison to our model with or without GPS data, that the LSTM cell brings more accuracy. 


We observed that using GPS data generally results in an improvement when considering important geomagnetic storms. 
Figure \ref{fig:gpnngpshalloween} presents forecasts produced the GPNN-GPS model in blue and GPNN model in red, 
...
of $\SI{-348}{\nano\tesla}$ while GPNN-GPS provides a prediction of $\SI{-405}{\nano\tesla}$. 

For a four hour ahead forecast, GPNN predicts a storm peak of $\SI{-335}{\nano\tesla}$ while GPNN-GPS predicts $\SI{-380}{\nano\tesla}$.
...
$\mathbb{P}[\SI{-250}{\nano\tesla} \leq \mathrm{Dst} \leq \SI{-50}{\nano\tesla}]$ is the probability of occurrence of an intense storm 
...
versus the true positive ratio (TPR). The FPR is the ratio of false positive divided by the total number of negatives. 
The TPR also called sensitivity is the ratio of true positives divided by the total number of positives. For perfect 
...
highest level of activity, meaning $\mathrm{Dst} < \SI{-250}{\nano\tesla}$ provide FPR for each threshold (the highest 
...
($\SI{-250}{\nano\tesla} < \mathrm{Dst} < \SI{-50}{\nano\tesla}$), the GPNN provides TPR higher than $0.670$ for thresholds 
...
real value. For example, for 5h ahead, the real peak of activity of \SI{-422}{\nano\tesla} has a predicted value of \SI{-391}{\nano\tesla}. 
